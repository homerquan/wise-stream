...
object RabbitRegistry {
  val inboundExchange = Exchange("wise.inbound.command_exchange", Direct, true)
  val inboundQueue = Queue("wise.inbound.command_queue")
}
 ...
 */
class EventUpdateReceiver  extends App with FlowFactory with LazyLogging {
